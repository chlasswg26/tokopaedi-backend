...
module.exports = {
  registerControllers: async (req, res) => {
    try {
      ...
      if (data?.role) {
        queryAdditionalDatabase = '(name, email, password, picture, role, verification_code) VALUES($1, $2, $3, $4, $5, $6)'
        queryValueDatabase = [
          data.name,
          data.email,
          hashPassword,
        ...
        queryAdditionalDatabase = '(name, email, password, picture, verification_code) VALUES($1, $2, $3, $4, $5)'
        queryValueDatabase = [
          data.name,
          data.email,
          hashPassword,
...
<p style="margin: 0; font-size: 14px; text-align: center;"><span style="background-color:transparent;">Thank you for registering on ${SITE_NAME}, you need to verification by clicking button below.</span></p>
</div>
</div>
</td>
</tr>
...
`
      ...
      const sendMailer = await transporter.sendMail({
        ...
        to: data.email,
        subject: `${SITE_NAME} Services: Account need verification`,
        html: emailTemplates
      })
  ...
  verificationControllers: async (req, res) => {
    try {
      ...
      const decryptVerificationCode = legacyDecrypt(data?.code)
...
<div style="font-family: sans-serif">
<div class="txtTinyMce-wrapper" style="font-size: 14px; font-family: Varela Round, Trebuchet MS, Helvetica, sans-serif; mso-line-height-alt: 21px; color: #ffffff; line-height: 1.5;">
<p style="margin: 0; font-size: 14px; text-align: center;"><span style="background-color:transparent;">Yaay! Your account has been verified.</span></p>
</div>
</div>
</td>
</tr>
...
`
      ...
      const sendMailer = await transporter.sendMail({
        from: EMAIL_SERVICE || 'support@demo.com',
        to: user.email,
        subject: `${SITE_NAME} Services: Verified account`,
        html: emailTemplates
      })
